        ...
        <div className="container mx-auto px-10 mb-8">
            <div className="border-b w-full inline-block border-blue-400 py-8">
                ...
                <div className={`${isMenuVisible ? 'max-h-full' : 'h-0'} overflow-hidden`}>
                    {categories.map((category, index) => (
                        <Link key={index} href={`/category/${category.slug}`} passHref>
                            <span id={`menu-${index}`} className="block sm:invisible mt-2 align-middle text-sm text-white ml-4 font-semibold cursor-pointer">
                                {category.name}
                            </span>
                        </Link>
                    ))}
                    ...
                    <Link href="/" passHref>
                        <span id="menu-about" className="block sm:invisible mt-2 align-middle text-sm text-white ml-4 font-semibold cursor-pointer">
                            About
                        </span>
                    </Link>
                </div>
                ...
                <div className="invisible sm:visible float-right justify-evenly">
                    {categories.map((category, index) => (
                        <Link key={index} href={`/category/${category.slug}`} passHref>
                            <span id={`menu-${index}`} className="mt-2 align-middle text-sm text-white ml-4 px-4 font-semibold cursor-pointer">
                                {category.name}
                            </span>
                        </Link>
                    ))}
                    ...
                    <Link href="/" passHref>
                        <span id="menu-about" className="mt-2 align-middle text-sm text-white ml-4 px-4 font-semibold cursor-pointer">
                            About
                        </span>
                    </Link>
                </div>
